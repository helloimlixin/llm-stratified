...
def _matplotlib_supports_3d() -> bool:
    if plt is None:
        return False
    try:
...
def compute_stratified_manifold_hypothesis_metrics(
    *,
    ...
    centers = np.column_stack(((bbox_np[:, 0] + bbox_np[:, 2]) * 0.5, (bbox_np[:, 1] + bbox_np[:, 3]) * 0.5))
    ...
    local_same_mask = same_image_mask & (np.take_along_axis(center_dists, nn_idx, axis=1) <= float(neighborhood_size))
    ...
    same_image_neighbor_top1 = float(np.mean(same_image_mask[:, 0])) if same_image_mask.shape[1] else float("nan")
    ...
    local_same_image_given_same_image_mean = float(np.mean(local_same_mask.sum(axis=1) / local_same_den))
    ...
    same_image_fiber_score = _clip01(((same_image_neighbor_lift if math.isfinite(same_image_neighbor_lift) else 0.0) - 1.0) / 9.0)
    ...
    hypothesis_score = float(np.mean([same_image_fiber_score, local_chart_score, smoothness_score, regularity_score]))
...
def _maybe_wandb_metric_table(wandb_module, prefix: str, metrics: Dict[str, Any]):
    ...
    if not rows:
        return None
    return wandb_module.Table(columns=["metric", "value"], data=rows)
        ...
        try:
            ...
            if irregular:
                ...
                irregular_samples = _wandb_image_or_none(
                    wandb_module=wandb_module,
                    key="embeddings/irregular_samples",
                    build_fn=lambda: [
                        wandb_module.Image(
                            add_heatmap_patch(
                                item["img"],
