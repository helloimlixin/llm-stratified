"""Lightweight geometric estimators for stratification and volume probes."""
...
def _geo_estimator(
    ...
    coeffs, residuals, _rank, _singular_values = np.linalg.lstsq(rstack, np.log(volumes), rcond=None)
    scaling_coeff = float(np.exp(coeffs[0]) / npts)
    dimension = float(coeffs[1])
    residual = float(residuals[0]) if residuals.size else 0.0
    if args.miller:
        scaling_coeff *= float(np.exp(0.5 * residual**2))
    ricci = float(np.mean(-residual * 6 * (dimension + 2) / radii**2)) if args.ricci else 0.0
...
def run_fiber_bundle_test_from_sorted_dists(
    ...
    *,
    ...
    return [
        _estimate_stratifications(dists_sorted[:, i], vol_min, vol_max, npts, args, ws=ws, alpha=alpha)
        for i in range(npts)
    ]
