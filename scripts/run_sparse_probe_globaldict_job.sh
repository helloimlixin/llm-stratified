...
if (( ${#DICT_LIST[@]} == 0 || ${#SEED_LIST[@]} == 0 )); then
...
dict_idx=$(( remaining % ${#DICT_LIST[@]} ))
remaining=$(( remaining / ${#DICT_LIST[@]} ))
...
GLOBAL_DICT="${DICT_LIST[$dict_idx]}"
TRAINING_SEED="${SEED_LIST[$seed_idx]}"

if [[ "$GLOBAL_DICT" == "true" ]]; then
  DICT_TAG="global"
else
  DICT_TAG="local"
fi
...
RUN_DIR="$OUT_ROOT/dict_${DICT_TAG}/seed_${TRAINING_SEED}/job_${JOB_BASE}_task_${TASK_INDEX}"
WANDB_RUN_NAME="sparse_probe_${DICT_TAG}_seed${TRAINING_SEED}_job${JOB_BASE}_task${TASK_INDEX}"
...
CMD=(
  ...
  "fiber.sparse_probe_global_dictionary=$GLOBAL_DICT"
)
...
echo "  global_dictionary=$GLOBAL_DICT ($DICT_TAG)"
