...
# Usage: ./run_multi_gpu.sh [dataset] [hydra_overrides...]
# Example: ./run_multi_gpu.sh CIFAR10 training.epochs=50 wandb.enabled=true wandb.project=tinyvit
...
torchrun --nproc_per_node=$NUM_GPUS "$ROOT_DIR/src/train.py" \
    compute=ddp \
    data="$DATASET_LC" \
    wandb.enabled=true \
    wandb.project=tinyvit \
    "$@"
