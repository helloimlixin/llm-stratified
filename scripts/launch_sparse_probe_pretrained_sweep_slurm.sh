...
JOB_NAME="${JOB_NAME:-sparse-probe-pretrained}"
...
if (( ${#MODEL_LIST[@]} == 0 || ${#DATASET_LIST[@]} == 0 || ${#SEED_LIST[@]} == 0 )); then
  echo "All sweep axes must be non-empty" >&2; exit 1
fi
...
rsync -a \
  --exclude '/.git/' --exclude '/runs/' --exclude '/wandb/' --exclude '/data/' \
  --exclude '__pycache__/' --exclude '*.pyc' --exclude '*.out' --exclude '*.err' \
  "$ROOT_DIR/" "$SNAPSHOT_REPO/"

chmod +x "$SNAPSHOT_REPO/scripts/run_sparse_probe_pretrained_job.sh"

export ROOT_DIR="$SNAPSHOT_REPO"
export DATA_ROOT OUT_ROOT MODELS DATASETS SEED_VALUES RESIDUAL_THRESHOLD NEIGHBOR_K
export EPOCHS EMBED_INTERVAL SUBSET_TEST CONDA_ENV
export WANDB_ENABLED WANDB_MODE WANDB_PROJECT
...
JOB_ID="$(sbatch --parsable "${SBATCH_ARGS[@]}" "$SNAPSHOT_REPO/scripts/run_sparse_probe_pretrained_job.sh" "$@")"
