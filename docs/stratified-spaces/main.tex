...
We further ask whether raw-patch sparse complexity changes as one moves along the empirical fiber coordinate. This auxiliary diagnostic uses a separate 50-epoch \dataset{STL10} trained TinyViT run. At each checkpoint, tokens are ordered by their first PCA coordinate, which we use as a simple one-dimensional fiber coordinate. We select anchors along this coordinate, gather the raw image patches in each anchor's local token neighborhood, and fit one shared 32-atom dictionary from the pooled patches. Each neighborhood is then coded against this same dictionary, and its sparse complexity is the mean number of atoms needed to reconstruct its patches to a fixed residual tolerance. The key control is the shared dictionary: if the required sparsity changes along the fiber coordinate, the change reflects that some regions are easier or harder for the same dictionary to represent, rather than a consequence of fitting a separate local dictionary for each region.
...
The global-dictionary run shows nonconstant sparse complexity along the fiber coordinate. Mean required sparsity remains high, about 13.3--14.5 atoms, and the sparsity trajectory has substantial total variation along the sorted fiber-coordinate values. The correlation between required sparsity and local dimension is positive at every reported checkpoint, ranging from 0.37 to 0.53. This supports the interpretation that high-dimensional token neighborhoods tend to correspond to raw-patch neighborhoods that require more shared dictionary atoms to reconstruct.

\paragraph{Color encoding in the sparse-probe plots.}
The point colors in \Cref{fig:sparse-global} are diagnostic encodings, not class labels. In the upper-right panel, color denotes mean required sparsity, matching the colorbar: brighter points require more dictionary atoms on average. In the lower two scatter plots, color denotes the number of raw patches in the anchor's $\epsilon$-ball neighborhood, using the displayed patch-count colorbars. This exposes whether an apparent relationship between local dimension, irregularity, and sparse complexity is also coupled to neighborhood support size; yellow-green points have more patches in the $\epsilon$-ball, while darker purple points have fewer. The first panel is a one-dimensional trajectory along the sorted fiber coordinate and uses a fixed line color.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{\figroot/sparse_global_epoch049.png}
\caption{Sparse dictionary probe at epoch 49 for the auxiliary global-dictionary run. The first panel shows the sparsity trajectory along the PCA-1 fiber coordinate. The upper-right scatter colors anchors by mean required sparsity, while the lower dimension/sparsity and irregularity/sparsity scatters color anchors by $\epsilon$-ball patch count.}
