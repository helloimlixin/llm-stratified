...
\paragraph{Local sparse-complexity probe.}
For trained-token diagnostics we add a local sparse dictionary probe that uses the same token neighborhoods but measures raw-patch reconstruction complexity. At checkpoint $t$, fix a radius $\epsilon_t$ in token space and define the neighborhood
\begin{equation}
  \mathcal{N}_i(\epsilon_t)=\{j:\|z_j-z_i\|_2 \le \epsilon_t\}.
\end{equation}
In the reported sweep, $\epsilon_t$ is the median 32-nearest-neighbor radius at that checkpoint, so the radius is fixed across all anchors within a checkpoint. We keep anchors with $|\mathcal{N}_i|\ge 12$. For each neighbor $j$, let $x_j\in\R^p$ be the corresponding raw image patch vector and let $\tilde{x}_j$ be its centered, unit-norm version. For each anchor $i$ we fit a local dictionary $D_i\in\R^{M_i\times p}$, $M_i\le 32$, from the top right singular vectors of the centered matrix $\{\tilde{x}_j:j\in\mathcal{N}_i\}$, with local patch mean $\mu_i$.
For each patch in the neighborhood, Orthogonal Matching Pursuit estimates the minimum code length needed to reach one global residual tolerance $\tau=0.15$:
\begin{equation}
  s_{ij}
  =
  \min\left\{
    s\le S_{\max}:
    \exists a,\ \|a\|_0\le s,\ 
    \frac{\|(\tilde{x}_j-\mu_i)-D_i^\top a\|_2}{\|\tilde{x}_j-\mu_i\|_2}
    \le \tau
  \right\},
  \qquad S_{\max}=16 .
\end{equation}
The token-level sparse complexity is
\begin{equation}
  S_i=\frac{1}{|\mathcal{N}_i|}\sum_{j\in\mathcal{N}_i} s_{ij}.
\end{equation}
We summarize the distribution of $S_i$ and its correlations with neighborhood support size $|\mathcal{N}_i|$, local dimension $\dimhat_i$, and irregularity $\irr_i$. This diagnostic deliberately uses one local dictionary per token neighborhood and no one-dimensional coordinate ordering.
...
\subsection{Auxiliary Token-Patch and Local Sparse Diagnostics}
...
\paragraph{Local sparse dictionary probe.}
We trained TinyViT on \dataset{STL10} for 50 epochs with three seeds and evaluated the local sparse-complexity probe at epochs $0,9,19,29,39,49$. The final evaluation accuracies were 60.9\%, 61.3\%, and 62.2\%, so the run is in a partially trained regime rather than a saturated classifier regime. \Cref{tab:local-sparse} reports means across seeds.

\begin{table}[H]
\centering
\scriptsize
\caption{Local sparse dictionary probe on trained \dataset{STL10} TinyViT tokens, averaged over three seeds. $\epsilon$ is fixed within each checkpoint as the median 32-nearest-neighbor token radius. $S_i$ is the mean OMP code length needed to reconstruct local raw patches to relative residual $\tau=0.15$; $\rho$ denotes Pearson correlation and $n_i=|\mathcal{N}_i|$.}
\begin{tabular}{@{}rccccccc@{}}
\toprule
Epoch & $\epsilon$ & Tokens & Mean $S_i$ & IQR & $\rho(S,n)$ & $\rho(S,\dimhat)$ & $\rho(S,\irr)$ \\
\midrule
0  & $1.95{\pm}0.30$ & 607 & $13.98{\pm}0.06$ & 1.44 & 0.70 & 0.27 & -0.04 \\
9  & $3.69{\pm}0.21$ & 725 & $12.95{\pm}0.62$ & 3.86 & 0.73 & 0.07 & -0.07 \\
19 & $5.54{\pm}0.32$ & 763 & $12.51{\pm}0.22$ & 4.65 & 0.74 & 0.06 & -0.07 \\
29 & $6.83{\pm}0.74$ & 814 & $12.22{\pm}0.22$ & 5.30 & 0.72 & 0.06 & -0.13 \\
39 & $7.42{\pm}0.64$ & 853 & $12.10{\pm}0.14$ & 5.36 & 0.77 & 0.08 & -0.12 \\
49 & $7.56{\pm}0.70$ & 846 & $12.13{\pm}0.10$ & 5.46 & 0.76 & 0.12 & -0.12 \\
\bottomrule
\end{tabular}
\label{tab:local-sparse}
\end{table}

Two effects are visible. First, mean sparse complexity drops during training, from $13.98$ atoms at initialization to about $12.1$ atoms by epoch 49. The learned token geometry therefore organizes local raw-patch neighborhoods into sets that are easier for their own local dictionaries to reconstruct. Second, the distribution broadens: IQR($S_i$) grows from 1.44 to 5.46, and the 10--90\% band at epoch 49 spans roughly 7.3--15.6 atoms. Training therefore does not make the space uniformly simple. Instead, some neighborhoods become much lower-complexity while a high-complexity tail persists.

The sparse-complexity signal is not just a restatement of the volume-scaling dimension. The strongest association is with fixed-radius support size, Corr($S_i,|\mathcal{N}_i|$) $\approx 0.7$--$0.77$, while Corr($S_i,\dimhat_i$) is weak after early training and Corr($S_i,\irr_i$) is mildly negative. Thus the local dictionary probe measures a complementary notion of local structure: how many raw-patch directions are needed for reconstruction at a fixed residual threshold, conditional on a neighborhood in token space. This is compatible with a stratified interpretation because the local complexity field is heterogeneous, but it also shows that sparse complexity, local dimension, and irregularity are not interchangeable statistics.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.49\linewidth}
  \centering
  \includegraphics[width=\linewidth]{\figroot/local_sparse_epoch049_probe.png}
  \caption{Summary scatters}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\linewidth}
  \centering
  \includegraphics[width=\linewidth]{\figroot/local_sparse_epoch049_heatmaps.png}
  \caption{Patch heatmaps}
\end{subfigure}
\caption{Representative local sparse dictionary diagnostics at epoch 49 for seed 1337. Left: the distribution of $S_i$ and its relation to support size, local dimension, irregularity, and residual error. Right: patch-level overlays of $S_i$ on source images, showing that sparse complexity varies spatially across token neighborhoods.}
\label{fig:local-sparse}
\end{figure}

\section{Discussion}
Four points stand out. First, raw pixel spaces are already highly stratified, so the interesting question is not whether learned models ``break'' a smooth manifold but how they redistribute singular structure already present in the data. This is the direct image-space analogue of the observation in \citet{robinson2025token}: continuity of the coordinates does not guarantee a locally smooth or even fiber-bundle-like data geometry. Moreover, the present image-space setting is arguably finer-grained than the token-embedding setting because it probes continuous local neighborhoods and multiple patch geometries rather than only a fixed vocabulary embedding. Second, tokenization does not automatically smooth local geometry: the untrained TinyViT tokens are more irregular than matched raw pixels on average. Third, the DINOv2 comparison shows that smoothing can happen at the patch-embedding stage even when later token layers become more singular again. Fourth, the local sparse dictionary probe shows that training can lower average raw-patch reconstruction complexity while increasing heterogeneity across token neighborhoods; this is the qualitative pattern expected from a representation that separates different local regimes rather than collapsing them to one uniform manifold chart.
...
\item The auxiliary token-patch figure comes from an earlier long trained-fiber visualization run, while the local sparse dictionary table and diagnostics use a separate 50-epoch \dataset{STL10} trained TinyViT sweep over three seeds.
